    ...
    def register():
        if request.method == "GET":
            ...
            if success:
                ...
                return redirect(url_for('user.profile', user_id=result.user_id, css_class="success", message=message))
            else:
                return render_template('auth/register.html', user=request.form, css_class="error", message=result)
    ...
    def login():
        if request.method == "POST":
            ...
            if success:
                ...
                return redirect(next_page or url_for('user.profile', user_id=user.user_id, css_class="success", message=message))
            else:
                message = "Invalid username or password"
                render_template('auth/login.html', css_class="error", message=message)
        else:
